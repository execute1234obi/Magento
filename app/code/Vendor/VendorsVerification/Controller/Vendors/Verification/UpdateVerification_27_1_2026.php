...
// }
if ($typeId == InfoGroup::VENDOR_VERIFICATION_DATAGROUP_CERTI_DOCS) {
    // Frontend keys: vendor_certificate aur vendor_documents
   
    
    
   /** * PHTML ke dataScope ke mutabiq keys 'certificate' aur 'documents' hain.
     * Magento UI uploader inhein hamesha array mein bhejta hai.
     */
...
//exit();
            ...
            if (!empty($processedData)) {
                //$this->saveInfoGroup($verificationInfo, $processedData);
                //print_r($verificationInfo);
                //print_r( $processedData);print_r($verificationId);print_r($typeId);exit();
                $this->saveInfoGroup($verificationInfo, $processedData, $verificationId, $typeId);
                $this->syncVendorFromVerification($vendor, $typeId, $processedData);
              //  $vendor->save();
              // save() se pehle ye likhein debug ke liye:
//echo "<pre>";
//print_r($verificationInfo->getData());
//die("Testing IDs");
                // 🔥 FLAG FOR OBSERVER
