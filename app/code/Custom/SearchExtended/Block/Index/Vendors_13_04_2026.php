...
//                 'entity_id',
//                 ['in' => !empty($filteredIds) ? $filteredIds : [0]]
//             );
//         }
//     }
...
public function getSearchCollection()
{
    ...
    try {
        ...
        if (!empty($vendorIds)) {
            $collection->getSelect()->orWhere('e.entity_id IN (?)', array_unique($vendorIds));
        }
    ...
    $collection->getSelect()->joinLeft(
        ['vv' => 'business_vendor_verification'],
    ...
    if ($verified == 1) {
        ...
        // unverified + जिनका record ही नहीं है
    ...
    /* ================= MEMBERSHIP FILTER (IMPORTANT) ================= */
    $today = date('Y-m-d H:i:s');

    $collection->getSelect()->joinLeft(
        ['vmt' => 'ves_vendor_membership_transaction'],
        'e.entity_id = vmt.vendor_id',
        []
    );

    // Latest transaction only
    $collection->getSelect()->where('vmt.transaction_id IN (
        SELECT MAX(t.transaction_id)
        FROM ves_vendor_membership_transaction t
        GROUP BY t.vendor_id
    )');

    // Expiry check
    // $collection->getSelect()->where(
    //     "DATE_ADD(vmt.created_at, INTERVAL vmt.duration MONTH) >= ?",
    //     $today
    // );
